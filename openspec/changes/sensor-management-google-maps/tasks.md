...
- [x] 1.1 Create `Sensor`, `SensorStatus`, `CreateSensorRequest`, and `UpdateSensorRequest` TypeScript models in `src/app/core/models/sensor.model.ts`
...
## 2. Google Maps Component Integration

- [x] 2.1 Install/configure `@angular/google-maps` and Google Maps API script loading mechanism in environment and app configurations
- [x] 2.2 Build reusable `SensorMapComponent` with custom colored markers (`ONLINE`, `OFFLINE`, `MAINTENANCE`), auto-centering bounds, and interactive InfoWindows
